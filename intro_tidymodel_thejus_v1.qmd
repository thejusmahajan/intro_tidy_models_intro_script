...
Necessary packages are loaded. We use the `penguins` data as we are acquainted with it in the prior chatpers.
...
We have seen the tidyverse principles. The tidymodels framework follows tidyverse philosophy and essentially is a collection of packages for `modeling` and `machine learning`. It provides a `consentent` and `modular` approach to the entire workflow - from preprocessing to evaluation of model's performance. It is noteworthy to say that `Nextflow`, an NGS workflow tool that you will see later in this course resembles tidymodel principles.
...
-   The workflow is more clean and easily debuggable
    ...
    Yardstick provides tidy metrics functions:
